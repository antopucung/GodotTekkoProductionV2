  ...
  useEffect(() => {
    const initializeEnterpriseFeatures = async () => {
      try {
        ...
        // A/B testing disabled for production - no experiments API endpoint
        ...
        // // Sentry.captureException(error) // Commented out for production build // Commented out for production build
      }
    }
...
function EnhancedSessionHandler() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session?.user) {
      ...
      // Clear user context
            // setUserContext({}) // Commented out for production build
